...
void main()
{
    ...
    // Reverse Sobel Colors
    vec3 reverse_sobel = vec3(0.0);
    reverse_sobel.x = 1 - orig_g.x;
    reverse_sobel.y = 1 - orig_g.y;
    ...
    if (type == 1) {
        ...
        if (isnan(t_new.x) || isnan(t_new.y) || isnan(t_new.z)) {
            ...
            float m = max(max(t_new.x, t_new.y), t_new.z);
            outColor = vec4(.5 * (vec3(m) + vec3(reverse_sobel)), 1.0) * (A + B + C + D);
        }
    }
    else if (type == 5) {
        // Acrylic Painting
        ...
        outColor = normalize(vec4(total * vec3(reverse_sobel), 1.0) * (A + B + C + D));
    }
    else {
        // Original Image
